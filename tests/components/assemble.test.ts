...
describe("components — assembly", () => {
  it("AC1: a tool is one annotated class referenced by an agent; the container injects its dependencies", async () => {
    const bundle = await bundleOf(Greetings);
...
describe("components — errors at assembly", () => {
  it("AC3: an unregistered provider names the component", async () => {
    @Bundle({ ...baseBundle, name: "no-providers", agents: [GreeterAgent], mcp: [FilesServer] })
    class NoProviders {}

    await expect(bundleOf(NoProviders)).rejects.toThrow(
      'GreetTool: "Greeter" is not registered in @Bundle({ providers })',
    );
  });
  ...
  it("AC3: a non-component in tools, a missing prompt file and an unknown prompt variable fail", async () => {
    ...
    const bundleWith = (agentClass: abstract new () => unknown) => {
      @Bundle({ ...baseBundle, name: "x", agents: [agentClass] })
      class B {}
      return B;
    };
    const prompt = new URL("./fixture/greeter.prompt.md", import.meta.url);

    await expect(bundleOf(bundleWith(agent(prompt, [Plain])))).rejects.toThrow(
      /Plain in an agent's tools is not a @Tool/,
    );
    await expect(
      bundleOf(bundleWith(agent(new URL("./fixture/nope.md", import.meta.url)))),
    ).rejects.toThrow(/prompt file not found/);
    await expect(bundleOf(bundleWith(agent(prompt)))).rejects.toThrow(
      /unknown prompt variable \{\{language\}\}/,
    );
    await expect(bundleOf(Plain)).rejects.toBeInstanceOf(ComponentError);
  });
