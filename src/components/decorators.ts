...
/** An MCP server the bundle connects to (listed in `@Bundle({ mcp })`). */
...
/** The module of a set of agents (Angular `@NgModule`-like). Assemble with `bundleOf`. */
export function Bundle(options: BundleMeta) {
  return <C extends Class>(value: C): C => {
    recordComponent(value, { kind: "bundle", meta: options });
    return value;
  };
}
