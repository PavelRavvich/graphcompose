...
const depsOf = (cls: Class): readonly Token[] => {
  ...
  if (meta?.kind === "tool" || meta?.kind === "injectable") return meta.meta.deps;
  return [];
};
    ...
    for (const dep of depsOf(cls)) {
      ...
      if (reg === undefined) {
        throw new ComponentError(
          `${tokenName(cls)}: "${tokenName(dep)}" is not registered in @Bundle({ providers })`,
        );
      }
...
export function createContainer(
  ...
  const resolve = (token: Token): unknown => {
    ...
    if (reg === undefined && token instanceof InjectionToken) {
      throw new ComponentError(`"${tokenName(token)}" is not registered in @Bundle({ providers })`);
    }
    const value =
      reg?.kind === "value"
        ? reg.value
