...
/** What a bundle creates per run of the app: the container, tool instances, knowledge sources. */
...
/** One container per bundle and app services: its tools, knowledge and index share instances. */
const containers = new WeakMap<BundleMeta, WeakMap<BundleServices, Container>>();
const containerFor = (bundle: BundleMeta, services: BundleServices): Container => {
  const perBundle = containers.get(bundle) ?? new WeakMap<BundleServices, Container>();
...
export const ragParts = (
  ...
  agents: readonly AgentMeta[],
  rags: readonly Class[],
): Required<Pick<AgentBundle, "knowledge" | "knowledgeBases">> => ({
  knowledge: (services: BundleServices) =>
    contextSources(agents, (cls) => containerFor(bundle, services).get(cls) as RagConnector),
  knowledgeBases: (services: BundleServices) =>
    rags.map((cls) => ({
