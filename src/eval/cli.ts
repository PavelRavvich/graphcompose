...
// npm run eval    -- [--config <b>] [--profile <p>] [--version <v>] [--limit N]
// npm run replay  -- [--config <b>] [--profile <p>] --version <v> [--limit N]
// npm run compare -- --config <b> --profiles base,<p>… [--golden <name> | --last N]
// npm run golden  -- add --config <b> --name <name> [--from-last N]
...
async function golden(): Promise<void> {
  if (positionals[1] !== "add" || values.name === undefined)
    throw new Error("usage: golden add --config <b> --name <name> [--from-last N]");
